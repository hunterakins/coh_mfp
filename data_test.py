...
def get_mult_el_reps(env, num_synth_els, v, T,fname='swell', adiabatic=False):
    """
    ...
    """
    folder=  'at_files/'
    r, z, synth_reps = form_replicas(env, rmin, rmax, grid_dr, v, T, num_synth_els, folder, fname, adiabatic=adiabatic)
    return r, z, synth_reps
...
class DataRun:
    def __init__(self, proj_str, num_snapshots, v, subfolder, num_freqs, num_synth_els):
    ...
    def run(self, wnc=False, **kwargs):
        if wnc==False:
            r_center, cov_t, r, z, outputs, max_vals = new_synth_stacked_bart(self)
        else:
            r_center, cov_t, r, z, outputs, max_vals, wnc_outs, wnc_max_vals = new_synth_stacked_bart(self, wnc=wnc, **kwargs)
...
def get_outputs(drp, wnc=False, **kwargs):
    ...
    for freq_ind, source_freq in enumerate(freqs):
        ...
        r, z, synth_reps = get_mult_el_reps(env, drp.num_synth_els, drp.v, T,fname=drp.proj_str,adiabatic=True)
        if source_freq == freqs[0]:
        ...
        for i in range(num_times):
            K_samp = K_samps[i,:,:]
            output, max_val = get_amb_surf(r, z, K_samp, synth_reps) # output is in db
        ...
        if wnc == True:
            wnc_out_freq = run_wnc(K_samps[:num_times, :,:], synth_reps, wn_gain)
...
def new_synth_stacked_bart(drp,wnc=False, **kwargs):
    """
...
class DRPRuns:
    ...
    """
    def __init__(self, proj_str, subfolder, num_snap_list, num_freq_list, num_synth_el_list, vv):
        self.proj_str = proj_str
        self.subfolder = subfolder
    ...
    def run_all(self, wnc=False, **kwargs):
        """
        ...
        for num_snaps in self.num_snap_list:
            for num_freqs in self.num_freq_list:
                for num_synth_els in self.num_synth_el_list:
                    if num_synth_els == 1:
                        v = -2.5
                        dr = DataRun(self.proj_str, num_snaps, v, self.subfolder, num_freqs, num_synth_els)
                        dr.run(wnc=wnc, **kwargs)
                        dr.save()
                    else:
                        for v in self.vv:
                            dr = DataRun(self.proj_str, num_snaps, v, self.subfolder, num_freqs, num_synth_els)
    ...
    def show_vel_corrs(self):
        """ Get the velocity amb surface """
        for num_snaps in self.num_snap_list:
            for num_freqs in self.num_freq_list:
                for num_synth_els in self.num_synth_el_list:
                    if num_synth_els == 1:
                        ...
                        max_val_arr = get_max_val_arr(self.vv, dr.r_center, max_val_list)
                        plt.figure()
                        plt.contourf(dr.cov_t, self.vv, max_val_arr)
            ...
            for num_freqs in self.num_freq_list:
                for num_synth_els in self.num_synth_el_list:
                    ...
                    for i in range(len(r_center)):
                        ...
                        plt.close(fig)
                    os.system('ffmpeg -loglevel quiet -r 3 -f image2 -s 1920x1080 -i ' + folder + '/%03d.png -vcodec libx264 -crf 25  -pix_fmt yuv420p ' + folder +  str(num_snaps) + '_' + str(num_synth_els) + '_' + str(num_freqs) + '.mp4')
...
if __name__ == '__main__':
    ...
    num_snap_list = [15]
    num_freq_list = [13]
    #num_synth_el_list = [1, 2]#, 10, 15]
    num_synth_el_list = [1, 15]
    #num_synth_el_list = [1]

    #drp = DataRun(proj_str, num_snaps, -2.5, subfolder, rcvr_stride, 2, 1)
    ##drp.run(wnc=True, **kwargs)
    #drp = load_dr(drp)
    #thing = drp.wnc_outs

    #with open('npy_files/' + proj_str + '/' + subfolder + '/' + 'wnc_test.pickle', 'rb') as f:
    ...
    dr_runs.run_all()
    dr_runs.save('quick_test') 
    #dr_runs.save('wnc_test')
    """
    output_list = []
    vv= [-2.5]
    for v in vv:
